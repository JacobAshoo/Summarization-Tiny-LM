...
# ── Data ───────────────────────────────────────────────────────────────────────
DATA_FILE      = "/home/jacob/release/final/train.csv"
VAL_ROWS       = 4000
...
MAX_STEPS        = 50_000     # total optimiser steps
...
LR_MIN_RATIO     = 0.5         # cosine decays to LR * LR_MIN_RATIO, not 0
GRAD_CLIP        = 1.0         # max gradient norm — prevents exploding gradients
LABEL_SMOOTHING  = 0.1         # prevents overconfident predictions

# ── Checkpointing ──────────────────────────────────────────────────────────────
CKPT_DIR             = "./checkpoints"
CKPT_EVERY           = 5000   # save a checkpoint every N optimizer steps
...
class SummarizationIterableDataset(IterableDataset):
    """
    Streams rows from a CSV file one at a time — never loads
    ...
    Row range:
        skip_rows / max_rows split a single file into train/val.
        Val: skip_rows=0,         max_rows=VAL_ROWS  (first 8k rows)
        Train: skip_rows=VAL_ROWS, max_rows=None      (rest)
    ...
    def __init__(self, path: str, rank: int, world_size: int,
                 skip_rows: int = 0, max_rows: int = None, loop: bool = True):
        self.path       = path
        self.rank       = rank
    ...
    def _iter_file(self, global_worker_id: int, total_workers: int):
        buffer = []
        kept = 0
        with open(self.path, newline="", encoding="utf-8") as fh:
            reader = csv.DictReader(fh)
            for row_idx, row in enumerate(reader):
                # Row range filter — split train/val from one file
                if row_idx < self.skip_rows:
                    continue
                if self.max_rows is not None and kept >= self.max_rows:
                    break

                # Each worker reads only its assigned rows
                if (kept % total_workers) != global_worker_id:
                    kept += 1
                    continue
                kept += 1
    ...
    def __iter__(self):
        ...
        while True:  # loop indefinitely for training; val breaks after one pass
            yield from self._iter_file(global_worker_id, total_workers)
            if not self.loop:
                break
...
@torch.no_grad()
def validate(model, loader, criterion, rank) -> float:
    """
    Runs validation on this rank over the full val set and returns
    the local average loss. Caller is responsible for all_reduce-ing
    across ranks.
    """
    model.eval()
    total_loss = 0.0
...
def main():
    ...
    # ── Datasets ──────────────────────────────────────────────────────────────
    # Single CSV split by row index: first VAL_ROWS for validation, rest for training.
    # Data was globally shuffled during preprocessing, so the split is random.
    train_dataset = SummarizationIterableDataset(
        DATA_FILE, rank, world_size, skip_rows=VAL_ROWS)
    val_dataset   = SummarizationIterableDataset(
        DATA_FILE, rank, world_size, skip_rows=0, max_rows=VAL_ROWS, loop=False)
    ...
    val_loader = DataLoader(
        ...
        pin_memory=True,
        prefetch_factor=2,
        persistent_workers=False,  # respawn each validation since loop=False
    )
