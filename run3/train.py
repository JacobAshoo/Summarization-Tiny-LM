...
GRAD_ACCUM_STEPS = 16          # effective batch = BATCH_SIZE * world_size * GRAD_ACCUM_STEPS
                               # = 4 * 2 * 16 = 128 sequences per optimizer step
MAX_STEPS        = 70_000     # total optimiser steps
...
# ── Sentence Shuffling (lead-bias mitigation, Grenander et al. 2019) ──────────
SHUFFLE_PHASE_RATIO = 2 / 3   # fraction of MAX_STEPS trained on shuffled sources
SENT_END_IDS        = {18}    # ⚠ VERIFY THIS — run:
                               #   from tokenizers import Tokenizer
                               #   tok = Tokenizer.from_file("/home/jacob/release/bpe_tokenizer.json")
                               #   print(tok.encode(".").ids)  → e.g. [13]
                               # Put that ID here. With your Whitespace+BPE, "." should be
                               # its own token at the end of every sentence-final word.
SHUFFLE_PHASE_STEPS = int(MAX_STEPS * SHUFFLE_PHASE_RATIO)
...
class SummarizationIterableDataset(IterableDataset):
    ...
    def __init__(self, files: list[str], rank: int, world_size: int,
                 shuffle_src: bool = False):
        self.files       = files
        self.rank        = rank
        self.world_size  = world_size
        self.shuffle_src = shuffle_src

    @staticmethod
    def _shuffle_sentences(token_ids: list[int]) -> list[int]:
        """Split token list on sentence-ending tokens and shuffle the chunks."""f
        chunks, current = [], []
        for tid in token_ids:
            current.append(tid)
            if tid in SENT_END_IDS:
    ...
    def _iter_file(self, path: str, global_worker_id: int, total_workers: int):
        ...
        with open(path, newline="", encoding="utf-8") as fh:
            ...
            for row_idx, row in enumerate(reader):
                ...
                # Shuffle source sentences to destroy positional cues
                if self.shuffle_src:
                    src_ids = self._shuffle_sentences(src_ids)
...
def train(model, shuffled_loader, original_loader, val_loader, criterion, rank):
    """
    Two-phase step-based training loop — runs until MAX_STEPS.

    Phase 1 (steps 1..SHUFFLE_PHASE_STEPS): train on shuffled sources.
        Destroys sentence position cues, forcing the model to learn content
        semantics rather than exploiting lead bias.
    Phase 2 (steps SHUFFLE_PHASE_STEPS+1..MAX_STEPS): fine-tune on originals.
        Reintroduces position and discourse cues so the model can balance
        content importance with positional signal.
    ...
    # Start in Phase 1 (shuffled data)
    current_phase = "shuffle"
    active_loader = shuffled_loader
    data_iter     = iter(active_loader)

    if rank == 0:
        print(f"Phase 1 (shuffled): steps 1–{SHUFFLE_PHASE_STEPS}")
        print(f"Phase 2 (original): steps {SHUFFLE_PHASE_STEPS+1}–{MAX_STEPS}")
    ...
    def next_batch():
        nonlocal data_iter, active_loader
        try:
    ...
    for global_step in range(1, MAX_STEPS + 1):
        ...
        # ── Phase switch ─────────────────────────────────────────────────────
        if current_phase == "shuffle" and global_step > SHUFFLE_PHASE_STEPS:
            current_phase = "finetune"
            active_loader = original_loader
            data_iter     = iter(active_loader)
            if rank == 0:
                print(f"\n{'='*60}")
                print(f"  Step {global_step}: switching to Phase 2 (original data)")
                print(f"{'='*60}\n")
                mlflow.log_metric("train/phase", 2, step=global_step)
        ...
        if rank == 0 and global_step % LOG_EVERY == 0:
            ...
            phase_tag  = "shuf" if current_phase == "shuffle" else "orig"
            print(
...
def main():
    ...
    # Phase 1: shuffled sources (destroys positional cues → learns content)
    # Phase 2: original sources (reintroduces position + discourse cues)
    shuffled_train_dataset = SummarizationIterableDataset(
        TRAIN_FILES, rank, world_size, shuffle_src=True)
    original_train_dataset = SummarizationIterableDataset(
        TRAIN_FILES, rank, world_size, shuffle_src=False)
    ...
    if rank == 0:
        ...
        mlflow.log_params({
            ...
            "grad_clip":            GRAD_CLIP,
            "label_smoothing":      LABEL_SMOOTHING,
            "shuffle_phase_ratio":  SHUFFLE_PHASE_RATIO,
            "shuffle_phase_steps":  SHUFFLE_PHASE_STEPS,
            "sent_end_ids":         str(SENT_END_IDS),
        })

    # ── Train ─────────────────────────────────────────────────────────────────
    try:
        train(model, shuffled_train_loader, original_train_loader,
              val_loader, criterion, rank)
    finally:
        if rank == 0:
            mlflow.end_run()
