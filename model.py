...
# ── Model Dimensions ───────────────────────────────────────────────────────────
D_MODEL    = 512
ENC_LAYERS = 8     # increased from 6 — capacity redistributed from FFN width to depth
DEC_LAYERS = 8     # increased from 6 — capacity redistributed from FFN width to depth
FFN_HIDDEN = 1365  # restored to canonical SwiGLU ratio: 8/3 * D_MODEL ≈ 1365
                   # previous 4096 (8:1 ratio) was over-wide; deeper+narrower generalises better
...
class SwiGLUFFN(nn.Module):
    ...
    Dropout applied after the activation, before the down projection —
    this regularises the largest parameter block in each layer.
    No bias anywhere.

    FFN(x) = down( Dropout( SiLU(gate(x)) * up(x) ) )
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, T, D_MODEL)
        return self.down(self.dropout(self.act(self.gate(x)) * self.up(x)))
...
class Seq2SeqTransformer(nn.Module):
    """
    Full Encoder-Decoder Transformer for Text Summarization.
    ...
    Weight tying:
      encoder embedding, decoder embedding, and output projection all share
      a single (VOCAB_SIZE × D_MODEL) weight matrix. Encoder and decoder
      embeddings are tied because they share a vocabulary; decoder embedding
      and output projection are additionally tied (Press & Wolf 2017) because
      they perform inverse operations. All three pointing at one matrix cuts
      ~51M parameters and improves token representation coherence.
    No bias anywhere.
    ...
    def __init__(self, gradient_checkpointing: bool = False):
        ...
        # Tie all three: encoder embed ↔ decoder embed ↔ output projection.
        # One shared matrix serves all three roles — smaller, more coherent.
        self.decoder.embed.weight = self.encoder.embed.weight  # enc ↔ dec embed
        self.out_proj.weight      = self.decoder.embed.weight  # dec embed ↔ out proj
